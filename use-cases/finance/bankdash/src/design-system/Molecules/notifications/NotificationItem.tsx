  ...
  return (
    <DropdownMenuItem
      ...
      className={clsx(
        ...
        "p-3  hover:bg-blue-100 focus:bg-blue-100 hover:text-primary transition-all"
      )}
    >
      <Link
        href={getActionUrl(actionUrl, action, status) || "#"}
        className="flex justify-between shrink-0 w-full items-start"
      >
        <div className="icon flex items-center">
          <Icon type={type} className="size-8 text-muted-foreground" />
        </div>
        <div className="flex flex-col ml-3 max-w-56 text-truncate">
          <Text as="span" className="font-semibold text-primary text-sm">
            {title}
          </Text>
